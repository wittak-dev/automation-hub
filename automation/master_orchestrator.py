...
class MasterOrchestrator:
    ...
    async def run(self):
        ...
        # Scheduling done in main() before async starts
    ...
    def schedule_tasks(self):
        ...
        print("🔧 DEBUG: schedule_tasks() called")
        ...
        try:
            ...
            if self.config.automation_schedule['autonomous_development']['enabled']:
                ...
                print(f"🔧 DEBUG: Scheduling autonomous dev at {dev_time} on {dev_days}")
                ...
                for day in dev_days:
                    try:
                        ...
                        print(f"  ✅ Scheduled for {day}")
                    except Exception as e:
                        print(f"  ❌ Failed to schedule {day}: {e}")
                ...
                print(f"✅ Autonomous development scheduled at {dev_time} on {dev_days}")
            ...
            if self.config.automation_schedule['weekly_reports']['enabled']:
                ...
                print(f"🔧 DEBUG: Scheduling weekly reports at {report_time} on {report_days}")
                ...
                print(f"✅ Weekly reports scheduled at {report_time} on {report_days}")
            ...
            if self.config.automation_schedule['partnership_check']['enabled']:
                ...
                print(f"🔧 DEBUG: Scheduling partnership checks at {partner_time} on {partner_days}")
                ...
                print(f"✅ Partnership checks scheduled at {partner_time} on {partner_days}")
            ...
            if self.config.automation_schedule['health_monitoring']['enabled']:
                ...
                print(f"🔧 DEBUG: Scheduling health monitoring every {interval} minutes")
                ...
                try:
                    ...
                    print(f"✅ Health monitoring scheduled every {interval} minutes")
                except Exception as e:
                    print(f"  ❌ Failed to schedule health monitoring: {e}")
                    self.logger.error(f"Failed to schedule health monitoring: {e}")
            
            print("🎉 schedule_tasks() completed successfully")
    ...
    async def send_daily_summary(self):
        ...
        Development:
        • Tasks Completed: {self.status.autonomous_dev['tasks_completed']}
        • PRs Created: {self.count_prs_today()}
        • Tests Passing: ✅
        
        Partnerships:
        • Active Conversations: {len(self.status.partnership_status.get('active_conversations', []))}
        • Actions Pending: {len(self.status.partnership_status.get('pending_actions', []))}
        
        System Health:
        • Status: {self.status.health_metrics['all_systems']}
        • Issues: {len(self.status.health_metrics['issues'])}
...
def main():
    ...
    print("""
    ╔══════════════════════════════════════════════════════════════╗
    ║          MASTER ORCHESTRATION SYSTEM v1.0                    ║
    ║                                                              ║
    ║  Coordinating autonomous development for:                    ║
    ║  • WhatsApp Analyser                                        ║
    ║  • HealthOS                                                 ║
    ║                                                              ║
    ║  Systems managed:                                            ║
    ...
    # Schedule tasks (MUST happen before async loop starts)
    ...
    try:
        ...
        print("\n\n👋 Orchestration stopped. All systems returning to manual control.")
    
    # Generate final summary
    print("\n📈 Session Summary:")
    print(f"  • Tasks Completed: {orchestrator.status.autonomous_dev['tasks_completed']}")
    print(f"  • Alerts Generated: {len(orchestrator.status.alerts)}")
    print(f"  • System Status: {orchestrator.status.health_metrics['all_systems']}")
    
    print("\n✅ Master Orchestrator shutdown complete.")
